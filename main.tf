...
#DNS zone and vnet link, resolver endpoint
resource "azurerm_private_dns_zone" "dnslab" {
  name                = "dnslab.com"
  resource_group_name = azurerm_virtual_network.hub-vnet.resource_group_name
}
resource "azurerm_private_dns_a_record" "spokerecord" {
  name                = "spokevm"
  zone_name           = azurerm_private_dns_zone.dnslab.name
  resource_group_name = azurerm_private_dns_zone.dnslab.resource_group_name
  ttl                 = 300
  records             = ["10.250.0.4"]
}
resource "azurerm_private_dns_zone_virtual_network_link" "hubdnslink" {
  name                  = "dnshubvnetlink"
  resource_group_name   = azurerm_private_dns_zone.dnslab.resource_group_name
  private_dns_zone_name = azurerm_private_dns_zone.dnslab.name
  virtual_network_id    = azurerm_virtual_network.hub-vnet.id
  registration_enabled = true
}
resource "azurerm_private_dns_zone_virtual_network_link" "spokednslink" {
  name                  = "dnsspokevnetlink"
  resource_group_name   = azurerm_private_dns_zone.dnslab.resource_group_name
  private_dns_zone_name = azurerm_private_dns_zone.dnslab.name
...
resource "azurerm_private_dns_resolver_inbound_endpoint" "inboundendpoint" {
...
resource "azurerm_subnet_network_security_group_association" "defaultsubnet" {
